...
const seedHospitals = async () => {
    try {
        ...
        // Check if hospitals already exist to avoid duplicates
        const count = await Hospital.countDocuments();
        if (count > 0) {
            console.log(`Database already has ${count} hospitals. Skipping seed to prevent duplicates.`);
            // Optional: Uncomment below to force clear and re-seed
            // await Hospital.deleteMany({});
            // console.log('Cleared existing hospitals.');
        } else {
            const result = await Hospital.insertMany(hospitals);
            console.log(`Successfully added ${result.length} hospitals!`);
        }
